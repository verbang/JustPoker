...
系统 SHALL 为非游戏状态下的断线玩家提供重连窗口（30 秒）。超时后 SHALL 将玩家从房间中移除。系统 SHALL 区分"主动离开"和"断线"两种情况，确保主动离开不触发重连等待。
...
- **THEN** 服务端 SHALL 将该玩家从房间中移除，广播 `ROOM_UPDATE` 和 `PLAYER_LEFT` 事件（reason: 'timeout'）

#### Scenario: 主动离开不触发重连
- **WHEN** 玩家主动发送 LEAVE_ROOM 事件
- **THEN** 系统 SHALL 不将玩家加入 disconnectedPlayers Map，直接从 roomPlayers 中移除玩家，广播 `PLAYER_LEFT` 事件（reason: 'leave'）
